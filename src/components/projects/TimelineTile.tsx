...
export const EvenTile = ({ episode, maxCharacters = 6, type = 'small' }: TileType) => (
  <div className="group flex">
    <div className={classNames(type == 'small' ? 'flex-1' : 'w-12 flex-none', 'hidden text-right font-mono md:block md:w-24')}>
      <span className="block text-xl md:text-3xl">{format(parse(episode.air_date, 'MMM dd, yyyy'), 'dd')}</span>
      <span className="md:text-normal text-sm">{format(parse(episode.air_date, 'MMM dd, yyyy'), "MMM ''yy")}</span>
    </div>
