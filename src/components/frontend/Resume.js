...
export default function Resume() {
  ...
  const mappedData = data && data.results ? data.results.map(job => {
    return {
      ...job,
        ...
        <div className="mt-5 mx-auto">
          ...
          <div className="flex group" key={`job-image-${index}`}>
            <div className="hidden md:block flex-1 md:w-36 font-mono text-right group-odd:hidden">
              <span className="text-xl md:text-2xl flex items-center justify-end gap-1">
            ...
            <div className="hidden md:block flex-none w-4 md:w-8 relative mx-2 md:mx-4 group-odd:hidden">
              ...
              <span className="absolute top-1 left-1/2 transform -translate-x-1/2 rounded-full w-4 h-4 bg-primary-200 border-2 border-primary-800 z-20 group-first:bg-emerald-500 group-last:bg-red-500 group-last:w-6 group-last:h-6 group-first:w-6 group-first:h-6"></span>
            </div>
            <div className="pb-16 flex-1 group-last:pb-0">
              <div className="relative box-card p-4">
                <h3 className="text-2xl font-header break-words mb-2">{job.name}</h3>
                {job.logo ? (
                  <SVG
                    alt={job.company}
                    className={' fill-primary-700 dark:fill-primary-200 mb-4'}
                ...
                { job.tags?.map(tag => (
                  <span key={`resume_tag_${tag.id}`} className="text-xs rounded px-2 py-1 bg-secondary-800 text-primary-100">{tag.name}</span>
                ))}
                </div>
            ...
            <div className="hidden md:block flex-1 md:w-36 font-mono text-right group-even:hidden">
              <span className="text-xl md:text-2xl flex items-center justify-start gap-1">
                { job.startYear != job.endYear ? (
                  <>
                  {job.startYear}
                  <ChevronDoubleRightIcon className="h-5" />
