...
import { deleteProject } from '@/actions/resume';
...
export default async function DeleteResumePage({ params }: Props) {
  ...
  const project = await pullSingle(id);
  if (!project) {
    ...
    <div className="p-6">
      <Card type="small" className="mb-10 flex justify-between gap-10">
        <h2 className="flex items-center text-2xl font-semibold">
          Resume Delete <ChevronRight className="mx-2" /> {project?.name}{' '}
        </h2>
      </Card>
      {project && (
        <Card className="mx-auto max-w-3xl">
          ...
          <p className="mb-4">
            You are about to delete <span className="font-bold underline">{project?.name}</span>. All associated data will be permanently removed.
          </p>
...
async function pullSingle(id: string) {
